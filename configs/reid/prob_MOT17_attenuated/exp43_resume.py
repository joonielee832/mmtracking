...
#? Experiment details
exp_dir = "prob_reid_mot17_train_exp43"
num_gpus = 4
...
model = dict(
    reid=dict(
        ...
        head=dict(
            ...
            loss_attenuation=dict(attenuated=True, alpha=0.1, epoch_step=epoch_step, iters_in_epoch=iters_in_epoch/(num_gpus*batch_size)),  #* configurable; 70938 is the number of iterations in one epoch for MOT17
            loss=dict(type='CrossEntropyLoss', loss_weight=1.0),
            loss_pairwise=dict(
                type='TripletJRLoss', margin=0.05, euclid_margin=0.3, loss_weight=1.0, beta=0.85),  #* margin, beta configurable
            loss_uncertainty=dict(
                type='FeatureUncertaintyLoss', margin_exp=5, loss_weight=0.1),    #* margin_exp and loss_weight configurable
