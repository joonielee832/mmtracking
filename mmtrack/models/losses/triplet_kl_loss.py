...
from mmtrack.core.utils import KLDiv


@LOSSES.register_module()
class TripletKLDivLoss(nn.Module):
    """Triplet loss with Jeffrey's divergence and hard positive/negative mining.
    ...
    def __init__(self, margin=0.3, loss_weight=1.0, hard_mining=True, **kwargs):
        super(TripletKLDivLoss, self).__init__()
        self.margin = margin
        self.ranking_loss = nn.MarginRankingLoss(margin=margin)
        self.loss_weight = loss_weight
        self.hard_mining = hard_mining
        self.distance_metric = KLDiv()

    def hard_mining_triplet_loss_forward(self, feats, feats_log_cov, targets):
        """
        Args:
    ...
    def forward(self, feats, targets, feats_log_cov, **kwargs):
        if self.hard_mining:
            return self.hard_mining_triplet_loss_forward(feats, feats_log_cov, targets)
        else:
